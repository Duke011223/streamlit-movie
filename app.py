...
def main():
    ...
    if 'is_admin' not in st.session_state:
        st.session_state.is_admin = False  # 관리자인지 여부
    ...
    with st.sidebar:
        ...
        if st.session_state.user:
            ...
            if st.session_state.is_admin:
                st.write("관리자 권한으로 로그인되었습니다.")
            if st.button("로그아웃"):
                st.session_state.user = None
                st.session_state.is_admin = False  # 로그아웃 시 관리자 상태 해제
                st.success("로그아웃 성공!")
        else:
            ...
            if choice == "로그인":
                ...
                if st.button("로그인"):
                    ...
                    if user:
                        ...
                        # 관리자 로그인 체크 (admin username을 미리 지정)
                        if username == "admin":
    ...
    with tab3:
        st.header("👤 내 정보 수정")
        if st.session_state.user:
            user_info = next((u for u in users if u['username'] == st.session_state.user), None)
            if user_info:
                st.text_input("사용자명", value=user_info['username'], disabled=True)
                new_password = st.text_input("새 비밀번호", type="password")
            ...
            st.info("로그인 후 본인 정보를 수정할 수 있습니다.")
