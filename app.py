...
def main():
    ...
    with tab1:
        ...
        if total_movies == 0:
            ...
            for _, movie in filtered_df.iloc[start_idx:end_idx].iterrows():
                ...
                elif 'poster_url' in movie and pd.notna(movie['poster_url']):
    ...
    with tab2:
        st.header("⭐ 추천 영화")
        if not ratings:
            st.warning("추천할 영화 데이터가 없습니다.")
        else:
            avg_ratings = pd.DataFrame(ratings).groupby('movie')['rating'].mean().sort_values(ascending=False)
            top_movies = avg_ratings.head(5).index
            st.subheader("평점이 높은 추천 영화:")
            for movie in top_movies:
                movie_details = df[df['title'] == movie].iloc[0]
                st.write(f"**{movie_details['title']}** ({movie_details['release_date']})")
                st.write(f"평균 평점: {'⭐' * int(avg_ratings[movie])} ({avg_ratings[movie]:.2f}/10)")
    ...
    with tab3:
        ...
        if not st.session_state.user:
            ...
            if not user_ratings:
                st.info("현재 활동 내역이 없습니다.")
            else:
                st.write("내가 평가한 영화 목록:")
                for r in user_ratings:
                    st.write(f"**{r['movie']}** - 평점: {r['rating']}/10")
                    if r.get('review'):
