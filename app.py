...
with tab2:
    ...
    if st.session_state.user:
        ...
        if user_ratings:
            ...
            # 정렬 기준 선택
            sort_by = st.selectbox("추천 영화 정렬 기준", ["사이트 평점 순", "평점 높은 순", "리뷰 많은 순"])
            ...
            if sort_by == "사이트 평점 순":
                ...
                for _, movie in recommended_movies.iterrows():
                    ...
                    if movie_ratings:
                        avg_rating = round(sum(movie_ratings) / len(movie_ratings), 2)
                        movie_avg_ratings[movie['title']] = avg_rating
                recommended_movies['site_rating'] = recommended_movies['title'].map(movie_avg_ratings)
                recommended_movies = recommended_movies.sort_values(by="site_rating", ascending=False)
            ...
            # 리뷰 많은 순으로 정렬
            elif sort_by == "리뷰 많은 순":
                movie_reviews_count = {}
                for _, movie in recommended_movies.iterrows():
                    movie_reviews = [r['review'] for r in ratings if r['movie'] == movie['title'] and r.get('review') is not None]
                    movie_reviews_count[movie['title']] = len(movie_reviews)
                recommended_movies['reviews_count'] = recommended_movies['title'].map(movie_reviews_count)
                recommended_movies = recommended_movies.sort_values(by="reviews_count", ascending=False)
            ...
            if not recommended_movies.empty:
                for _, movie in recommended_movies.head(5).iterrows():
                    ...
                    poster_path = os.path.join(poster_folder, movie.get('poster_file', ''))
                    if os.path.exists(poster_path) and pd.notna(movie.get('poster_file')):
                        st.image(poster_path, width=200)  # 영화 포스터 이미지
                    else:
                        st.write("포스터 이미지가 없습니다.")
                    st.write(f"**장르**: {movie['genre']}")
                    st.write(f"**사이트 평점**: {round(movie['site_rating'], 2)}")
                    st.markdown("---")
            else:
        ...
        if st.session_state.user:
            ...
            if user_rated_movies:
                for r in user_rated_movies:
                    ...
                    # 기존 리뷰를 보여주고 수정할 수 있게 하기
                    review = r['review'] if r['review'] else '없음'
                    st.write(f"**리뷰**: {review}")
                
                    # 리뷰 수정 입력 필드
                    new_review = st.text_area(f"리뷰를 수정하세요 ({r['movie']})", value=review, key=f"edit_review_{r['movie']}")
                
                    # 리뷰 수정 버튼
                    if st.button(f"'{r['movie']}' 리뷰 수정", key=f"edit_button_{r['movie']}"):
                        # 리뷰가 수정된 경우에만 업데이트
                        if new_review != r['review']:
                            r['review'] = new_review if new_review else None  # 리뷰를 수정하거나 삭제
                            save_ratings(ratings)  # 변경사항 저장
                            st.success(f"'{r['movie']}' 영화의 리뷰가 수정되었습니다.")
                        else:
                            st.info("수정된 내용이 없습니다.")

                    # 영화 포스터 표시
                    movie_info = df[df['title'] == r['movie']].iloc[0]
                    poster_path = os.path.join(poster_folder, movie_info.get('poster_file', ''))
                    if os.path.exists(poster_path) and pd.notna(movie_info.get('poster_file')):
                        st.image(poster_path, width=200)  # 영화 포스터 이미지
                    else:
                        st.write("포스터 이미지가 없습니다.")
                    st.markdown("---")
