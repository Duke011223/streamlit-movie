...
def main():
        ...
        if st.session_state.user:
            ...
            if user_ratings:
                ...
                if sort_by == "가장 평점 높은 순":
                    ...
                    for movie in recommended_movies['title']:
                        ...
                        if movie_ratings:
                            ...
                            movie_avg_rating[movie] = 0  # 평점이 없을 경우 0으로 설정
                ...
                for _, movie in recommended_movies.iterrows():
                    st.subheader(movie['title'])
                    st.write(f"장르: {movie['genre']}, 평점: {movie.get('rating', '정보 없음')}")
                    poster_path = os.path.join(poster_folder, movie.get('poster_file', ''))
    ...
    with tab5:
        ...
        if st.session_state.role == 'admin':
            st.write("관리자 모드입니다.")
            st.write(f"사용자 목록: {users}")
            st.write(f"평가 목록: {ratings}")
        
            # 리뷰 수정 기능 추가
            selected_review = st.selectbox("수정할 리뷰를 선택하세요", options=[(r['username'], r['movie'], r['review']) for r in ratings if 'review' in r])
            if selected_review:
                new_review = st.text_area("리뷰 수정", value=selected_review[2])
                if st.button("리뷰 업데이트"):
                    for r in ratings:
                        if r['username'] == selected_review[0] and r['movie'] == selected_review[1]:
                            r['review'] = new_review
                            save_ratings(ratings)
                            st.success("리뷰가 업데이트되었습니다.")
        else:
