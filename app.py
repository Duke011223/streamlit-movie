...
def main():
        ...
        if st.session_state.user:
            ...
            if user_ratings:
                ...
                if sort_by == "가장 평점 높은 순":
                    ...
                    movie_avg_rating = {movie: round(sum([r['rating'] for r in ratings if r['movie'] == movie]) / len([r for r in ratings if r['movie'] == movie]), 2) if len([r for r in ratings if r['movie'] == movie]) > 0 else 0 for movie in recommended_movies['title']}
                ...
                st.write(f"추천 영화 목록 ({len(recommended_movies)}개)")
                ...
                for _, movie in recommended_movies.iterrows():
                    ...
                    st.write(f"**영화 평점**: {movie['rating']}")
                    st.markdown("---")
                    st.button("자세히 보기", key=f"details_{movie['title']}")

                    # 상세 정보 보기 처리
                    if st.session_state.user:
                        if st.button(f"'{movie['title']}' 자세히 보기", key=f"details_{movie['title']}"):
                            st.write(f"**제작사**: {movie['distributor']}")
                            st.write(f"**감독**: {movie['director']}")
                            st.write(f"**배우**: {movie['actor']}")
                            st.write(f"**장르**: {movie['genre']}")
                            ...
                            st.write(f"**리뷰**: {movie['review']}")
                            st.write(f"**상태**: {movie['running_state']}")
    ...
    with tab3:
        ...
        if st.session_state.user:
            user_ratings = [r for r in ratings if r['username'] == st.session_state.user]
            if user_ratings:
                st.subheader(f"**{st.session_state.user}님의 리뷰**")
    ...
    with tab4:
        ...
        if st.session_state.user:
            ...
            st.write("로그인 후 계정 관리 기능을 사용하세요.")
