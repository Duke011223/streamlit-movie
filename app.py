...
def main():
        ...
        if st.session_state.user:
            ...
            if user_ratings:
                ...
                if sort_by == "가장 평점 높은 순":
                    ...
                    recommended_movies = recommended_movies.loc[recommended_movies['title'].isin([r['movie'] for r in ratings])]

                recommended_movies = recommended_movies.head(1)

                if not recommended_movies.empty:
                    movie = recommended_movies.iloc[0]
                    st.subheader(movie['title'])
    ...
    with tab3:
        ...
        if st.session_state.user:
            user_ratings = [r for r in ratings if r['username'] == st.session_state.user]
            if user_ratings:
                st.subheader("나의 리뷰 및 평점")
                for rating in user_ratings:
                    st.write(f"**영화**: {rating['movie']}")
                    st.write(f"**평점**: {rating['rating']}⭐")
                    st.write(f"**리뷰**: {rating['review'] if rating.get('review') else '없음'}")
                    st.markdown("---")
                    # 리뷰 수정
                    new_review = st.text_area(f"수정할 리뷰 ({rating['movie']})", value=rating['review'] or "")
                    if st.button(f"수정 저장 ({rating['movie']})", key=f"update-review-{rating['movie']}"):
                        for r in ratings:
                            if r['username'] == st.session_state.user and r['movie'] == rating['movie']:
                                r['review'] = new_review
                        save_ratings(ratings)
                        st.success(f"리뷰가 수정되었습니다: {new_review}")
            else:
                st.write("아직 평점이나 리뷰를 남긴 영화가 없습니다.")

    # 사용자 계정 관리
    with tab4:
        st.header("🔧 사용자 계정 관리")
        if st.session_state.role == 'admin':
            st.write("관리자 권한으로 접근 중입니다.")
            username_to_delete = st.text_input("삭제할 사용자명")
            if st.button("사용자 삭제"):
    ...
    with tab5:
        ...
        if st.session_state.role == 'admin':
            st.write("관리자 대시보드:")
            st.write("모든 사용자와 영화 데이터를 볼 수 있습니다.")
            st.subheader("영화 목록")
            st.dataframe(df)
            st.subheader("사용자 목록")
