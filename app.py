        ...
        if st.session_state.user:
            ...
            if user_ratings:
                ...
                if sort_by == "사이트 평점 순":
                    ...
                    recommended_movies['avg_rating'] = recommended_movies['title'].map(movie_avg_ratings)
                    recommended_movies = recommended_movies.sort_values('avg_rating', ascending=False)
                ...
                # 리뷰 많은 순으로 정렬
                    ...
                    review_count = {movie: len([r for r in ratings if r['movie'] == movie]) for movie in recommended_movies['title']}
                    recommended_movies['review_count'] = recommended_movies['title'].map(review_count)
                    recommended_movies = recommended_movies.sort_values('review_count', ascending=False)

                st.write("추천된 영화 목록:")
                for _, movie in recommended_movies.iterrows():
                    st.subheader(movie['title'])
                    st.write(f"평균 평점: {movie['rating']}")
        ...
        if st.session_state.user:
            ...
            if user_ratings:
                for rating in user_ratings:
                    movie = df[df['title'] == rating['movie']].iloc[0]
                    st.write(f"영화: {rating['movie']}, 평점: {rating['rating']}점")
                    if rating.get('review'):
                        st.write(f"리뷰: {rating['review']}")
                    st.markdown("---")
            else:
                st.write("아직 평가한 영화가 없습니다.")
        else:
            st.write("로그인 후 활동을 확인할 수 있습니다.")
    ...
    with tab4:
        ...
        if st.session_state.user:
            if st.session_state.role == "user":
                st.write("사용자 정보 변경은 관리자만 할 수 있습니다.")
            else:
                st.write("관리자 기능은 여기서 처리됩니다.")

    # 관리자 보기
    with tab5:
        st.header("👑 관리자 보기")
        if st.session_state.role == "admin":
            st.write("관리자만 볼 수 있는 화면입니다.")
        else:
